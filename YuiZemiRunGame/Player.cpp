...
const float BodyLength = 5.f;
const float HandLength = 3.75f;
const float BodySize = 1.f;
const float HandSize = 0.625f;
const VECTOR DownVec = VGet(0, -1, 0);
const float MoveAnimSpeed = 2.f;
...
const float ConeR = 0.75f;
const float ConeSize = 1.5f;
...
void Player::MoveAnim(float deltaTime)
{
	if (moveAnimFlag)
	{
		...
		if (bodyPos.y +10.0f> BodyLength + MoveAnimTop)
		{
			moveAnimFlag = false;
		}
